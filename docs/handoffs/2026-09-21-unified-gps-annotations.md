...
- Projected 153 PDF hazards, equipment and places into `web/data/full-map-geographic.geojson` using the four rescue-equipment numbers shared with the existing GPS data. Existing GPS positions for those four stations and the strong-current area are preserved.
- Reduced overview marker size, hid document places until zoom 15, and identified projected positions as approximate in the legend and popups. The source document page is no longer linked or packaged offline.
...
- Replaced the visibly misaligned PDF road traces with 384 geographic OpenStreetMap roads and paths covering the coast. Place markers retain their smooth local adjustment; hazard and equipment annotations retain their earlier station-based fit.
...
2. Toggle each layer in the upper-right legend. Turning proposals off removes the dashed proposal symbols; turning it on restores them. Zoom in to see local OpenStreetMap roads, paths and projected place markers.
...
The four station controls span only the Chiquita area. Their fit residuals are 1.4 to 9.2 metres, but this is not independent accuracy. The place adjustment uses Banana Azul, Rocking J's, Congo Bongo and Maxi's as manual controls from OpenStreetMap/Nominatim (© OpenStreetMap contributors, ODbL); fitting those points does not validate other places. The previously tested Aguas Claras place remains about 55 metres north of the OSM point. The earlier background-image matching attempt failed holdout validation. Obtain independent landmark checks and field confirmation of hazards/equipment before treating PDF-derived GPS positions as precise or using them for navigation or rescue dispatch. OpenStreetMap roads do not establish access rights, current condition or passability.

## Files touched

`web/index.html`, `web/coastal.css`, `web/geometry.js`, `web/sw.js`, `web/data/full-map-geographic.geojson`, `web/data/osm-roads.geojson`, `tools/project_full_annotations.py`, `tools/fetch_osm_roads.py`, and `docs/design/icon-system-and-source-review.md`. PR cleanup also updates `README.md`, `docs/deploy.md`, `tools/build_map.js`, `tools/extract_full_map.py`, `web/_headers` and `web/vercel.json`, and removes `web/source-map.html`, `web/source-map.js` and `web/data/full-map-background.jpg` from the deploy folder.
