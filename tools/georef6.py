...
def main():
    ...
    # SYMMETRIC chamfer. One direction alone (your coast -> true coast) is
    # minimised by shrinking: a small footprint sits entirely near the shore and
    # scores well while explaining almost nothing. The reverse term (true coast
    # inside the footprint -> your coast) punishes exactly that, so the two
    # together have a real minimum at the true scale.
    rows = []
    for km in np.arange(5.0, 18.01, 0.25):
        ...
        for deg in np.arange(-8.0, 8.01, 0.5):
            ...
            foot = (cv2.warpAffine(np.ones((bh, bw), np.float32), R, (nw, nh)) > 0.5)
            if nh + 2 >= D.shape[0] or nw + 2 >= D.shape[1] or rot.sum() < 200:
                continue

            A_all = cv2.matchTemplate(D, rot, cv2.TM_CCORR) / rot.sum()
            a_err, _, loc, _ = cv2.minMaxLoc(A_all)

            canvas = np.zeros_like(D); fcv = np.zeros_like(D)
            h = min(canvas.shape[0]-loc[1], nh); w = min(canvas.shape[1]-loc[0], nw)
            canvas[loc[1]:loc[1]+h, loc[0]:loc[0]+w] = rot[:h, :w]
            fcv[loc[1]:loc[1]+h, loc[0]:loc[0]+w] = foot[:h, :w]
            if canvas.sum() < 200:
                continue
            Db = cv2.distanceTransform((1 - (canvas > 0.5).astype(np.uint8)), cv2.DIST_L2, 5)
            sel = (ref_c > 0) & (fcv > 0.5)
            if sel.sum() < 150:
                continue
            b_err = float(Db[sel].mean())
            rows.append(((a_err + b_err) / 2 * ref_mpp, km, deg, bw, loc, R.copy(),
                         a_err * ref_mpp, b_err * ref_mpp))

    if not rows:
        sys.exit("no candidates")
    rows.sort()
    print("\nsymmetric coastline error, best 10 (metres):")
    for err, km, deg, bw, loc, _, ae, be_ in rows[:10]:
        print(f"  {km:5.2f} km  {deg:+5.1f} deg  ->  {err:7.1f} m   "
              f"(yours->true {ae:6.1f}, true->yours {be_:6.1f})")

    # the profile matters as much as the winner: a real solution is a clear dip
    best_per_km = {}
    for err, km, *_ in rows:
        best_per_km[km] = min(best_per_km.get(km, 1e9), err)
    prof = sorted(best_per_km.items())
    print("\nerror vs assumed span (m):")
    print("  " + "  ".join(f"{k:.0f}:{e:.0f}" for k, e in prof[::4]))

    err, km, deg, bw, loc, R, a_err, b_err = rows[0]
